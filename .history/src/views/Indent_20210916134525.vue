      ...
      <div class="often">
        ...
        <div class="swiper-container">
          <div class="swiper-wrapper">
            <div class="swiper-slide" @click="again()">
              <li>
                <img v-lazy="img.j" />
                <div class="gray">
                  <p>俏九州小炒黄牛肉</p>
    ...
    <section class="order">
      ...
      <div class="orderList">
        <div class="orderListBox" v-show="0 === number">
          ...
          >
            <div class="orderAll">
              <div class="tou">
                <img v-lazy="coms.src" />
                <div class="orderInfo">
                  <p @click="again()">{{ coms.title }}></p>
                  <div class="discounts">
                    <div>
                      <span>{{ coms.ssows }}</span>
                    </div>
                    <div>{{ coms.houbao }}</div>
                    <div>{{ coms.discount }}</div>
                  </div>
                </div>
          ...
          <!-- 已取消 -->
          <a href="javascript:void(0)" v-for="fail in failList" :key="fail.id">
            <div class="orderAll">
              <div class="tou">
                <img :src="fail.src" alt="" />
                <div class="orderInfo">
                  <p @click="again()">{{ fail.title }}</p>
                  <div class="discounts">
                    <div>
                      <span>{{ fail.ssows }}</span>
                    </div>
                    <div>{{ fail.houbao }}</div>
                    <div>{{ fail.discount }}</div>
                  </div>
                </div>
  ...
  methods: {
    ...
    getsesucceedList() {
      this.axios.get("http://localhost:3000/indent").then((res) => {
        ...
        for (var i = 0; i < res.data.com.length; i++) {
          if (res.data.com[i].result == "已完成") {
            this.succeedList.push(res.data.com[i]);
          } else if (res.data.com[i].result == "已取消") {
            this.failList.push(res.data.com[i]);
          } else if (res.data.com[i].result == "待评价") {
            this.evaluateList.push(res.data.com[i]);
          }
        }
      });
    },
