          ...
          <a href="javascript:void(0)" v-for="com in getCom" :key="com.lenght">
            <div class="orderAll">
              ...
              <div class="right">
                ...
                <button @click="$router.push({path:'evaluate',query:{id:com.id}})">评价得40金豆</button>
              </div>
            </div>
          </a>
        </div>
...
export default {
  ...
  methods: {
    ...
    getsesucceedList(){
      this.axios.get('http://localhost:3000/indent').then(res=>{
        this.suclength = res.data[0].com.result
        for(var i=0; i<this.suclength.length;i++){
          if(this.suclength[i] == '已完成'){
            console.log(this.suclength)
          }
        }
      })
    }
