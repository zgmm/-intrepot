<template>
  <div class="Evaluate">
    ...
    <div class="merchant">
      <div class="hemlet">
        <div class="picbox"><img src="/images/思密达.jpg" alt="" /></div>
        <p>家里风味湘菜馆(南苑店)</p>
        <p><input type="radio" />匿名提交</p>
      </div>
      <div class="star">
        <van-rate v-model="value" :size="30" @change="onChange" />
        <p>{{ star }}</p>
        <div class="star-list">
          <div>
            <span class="bao">包装</span>
            <van-rate v-model="packaging" :size="25" @change="onChanges" />
            <span>{{ baozhuang }}</span>
          </div>
          <div>
            <span class="bao">味道</span>
            <van-rate v-model="flavour" :size="25" @change="onChanged" />
            <span>{{ weidao }}</span>
          </div>
          <textarea
            name=""
...
export default {
  ...
  mounted() {
      this.axios.get("http://localhost:3000/indent?index=0" + this.$route.query.id).then(res=>{
          this.getinfo = this.res.data;
      })
  },
};
</script>
