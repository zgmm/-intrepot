  ...
  methods: {
    ...
    shiyishi() {
        ...
        for (var i = 0; i < this.getinfo.com.length; i++) {
          if (this.lis.com[i].id == this.pingId) {
            this.lis.com[i].result = "已完成";
            break;
          }
        }
        this.axios.put("http://localhost:3000/indent",{params:this.lis}).then(()=>{
          this.$router.replace("/takeaway");
        })
    },
    //设置跟路由id相同的图片路径
    setinfosrc(id) {
      console.log( this.getinfo.com.length)
      for (var i = 0; i < this.getinfo.com.length; i++) {
        // console.log(this.getinfo[i].id);
        if (id == this.getinfo.com[i].id) {
          return this.getinfo.com[i].src;
        }
      }
    },
    //设置跟路由id相同的标题
    setinfotitle(id) {
      for (var i = 0; i < this.getinfo.com.length; i++) {
        // console.log(this.getinfo[i].id);
        if (id == this.getinfo.com[i].id) {
          return this.getinfo.com[i].title;
        }
      }
    },
  },
