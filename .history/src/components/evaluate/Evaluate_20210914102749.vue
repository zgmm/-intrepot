<template>
  <div class="Evaluate">
    <header>
      <span class="iconfont icon-AS" @click="$router.back(-1)"></span>
      <span>评价有礼</span>
    </header>
    <!-- 骑手部分 -->
    <div class="rider">
      <div class="top">
        <div class="pic">
          <img src="/images/waimai.png" alt="" />
        </div>
        <div class="man">
          <p>蜂鸟快送</p>
          <span>吴川 12:30~12:40准时送达</span>
        </div>
        <p class="anonymity">已对骑手匿名</p>
      </div>
      <div class="expression">
        <div class="bad active" @click="change(0)" :class="{active:0 === number}">
          <p><img src="/images/bad.png" alt=""></p>
          <p>非常差</p>
        </div>
        <div class="ordinary" @click="change(0)" :class="{active:0 === number}">
          <p><img src="/images/ordinary.png" alt="" /></p>
          <p>一般</p>
        </div>
        <div class="praise" @click="change(0)" :class="{active:0 === number}">
          <p><img src="/images/praise.png" alt="" /></p>
          <p>超赞</p>
        </div>
      </div>
    </div>
    <!-- 商家部分 -->
    <div class="merchant"></div>
  </div>
</template>


<script>
export default {
  name: "",
  computed: {},
  props: [],
  data() {
    return {};
  },
  methods: {},
  mounted() {},
};
</script>

<style scoped>
.Evaluate {
  width: 100%;
  height: auto;
  background: #f1f1f1;
}
header {
  text-align: center;
  width: 100%;
  z-index: 2;
  height: 0.9rem;
  font-size: 0.3rem;
  line-height: 0.9rem;
  color: #fff;
  position: fixed;
  background: #3190e8;
  font-weight: bold;
  top: 0;
  left: 0;
}
header span:nth-child(1) {
  position: absolute;
  left: 0.31rem;
  top: 0.03rem;
  font-size: 0.4rem;
}
.rider {
  margin: 0 auto;
  width: 95%;
  height: auto;
  background: #fff;
  border-radius: 0.1rem;
  margin-top: 0.9rem;
  padding: 0.2rem 0;
}
.top {
  width: 100%;
  height: 1.2rem;
  font-size: 0.25rem;
  display: flex;
  align-items: center;
  padding: 0.1rem;
  box-sizing: border-box;
}
.pic {
  width: 0.85rem;
  height: 0.85rem;
  border-radius: 50%;
  text-align: center;
  line-height: 1rem;
  margin-right: 0.15rem;
}
.pic img {
  width: 100%;
  background: #84baec;
  border-radius: 50%;
}
.man {
  width: 60%;
}
.man p {
  font-weight: bold;
  font-size: 0.25rem;
  color: #444;
}
.man span {
  font-size: 0.22rem;
  color: #aaa;
}
.anonymity {
  font-size: 0.21rem;
  margin-top: -0.4rem;
  color: #aaa;
}
.expression{
    width: 100%;
    height: 1rem;
    display: flex;
    justify-content: center;
    align-items: center;
    margin-top: .25rem;
}
.expression div{
    width: 1.3rem;
    padding: .1rem 0;
    text-align: center;
    border-radius: .1rem;
    margin: 0 .15rem;
}
.expression div img{
    width: .7rem;
}
.expression p{
    font-size: 0.25rem;
    line-height: .35rem;
}
.active{
    background: #84baec;
}
</style>
