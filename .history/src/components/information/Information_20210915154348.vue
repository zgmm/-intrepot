      ...
      <ul class="list-ul">
        ...
        <li class="tts" v-for="item in infoList" :key="item.id">
          <div class="box">
            ...
            <div class="delete" @click="remove(item.id)">删除</div>
          </div>
        </li>
      </ul>
    </section>
  ...
  methods: {
    ...
    delete() {
      ...
      for (let i = 0; i < this.infoList.length; i++) {
        /* 获取li开始的位置 */
        this.infoList[i].addEventListener("touchstart", function (e) {
          star = e.changedTouches[0].clientX;
        });
        this.infoList[i].addEventListener("touchmove", function (e) {
        ...
        this.infoList[i].addEventListener("touchend", function () {
          del[i].addEventListener("click", function () {
            this.infoList[i].remove();
          });
        });
      }
    },
