...
export default {
  ...
  methods: {
    searchOrder(){
      this.searchHistory = false;
      this.searchHistory.push(this.content);
      this.newsearchHistory = [];
      this.goodsList.forEach(item =>{
        if(item.title.indexOf(this.content) == -1 || item.goods.indexOf(this.content) == -1){
          this.without = true;
        }else if(this.content == ''){
          return;
        }else{
          this.newsearchHistory.push(item);
          this.record = this.newsearchHistory
        }
      });
      if(this.newsearchHistory.length > 0 || this.content == ''){
        this.without = false;
        this.searchHistory = true;
      }else{
        this.without = true;
