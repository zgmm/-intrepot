<template>
  <div class="details">
    ...
    <section class="recently">
      <div class="often">
        <p class="title">{{ state }}<van-icon name="arrow" /></p>
        <div class="box">
          <p class="baoxian">
          ...
          <h2>感谢信任，欢迎再次光临</h2>
          <div class="operate">
            <div class="pingjia" @click="pingjiaye()">
...
export default {
  ...
  methods: {
    ...
    getinfotitle(id) {
      for (let i = 0; i < this.dataList.length; i++) {
        if (id == this.dataList[i].id) {
          ...
          if (this.dataList[i].result == "已取消") {
            this.state = "订单已关闭";
          } else if (this.dataList[i].result == "待评价") {
            this.state = "订单已送达";
          } else if (this.dataList[i].result == "已完成") {
            this.state = "订单已完成";
          }
        }
      }
    ...
    pingjiaye(id) {
      for (let i = 0; i < this.dataList.length; i++) {
        if (id == this.dataList[i].id) {
          if (this.dataList[i].result == "待评价") {
            this.$router.push({ path: "evaluate", query: { id: shangping.id } });
          } else if (this.dataList[i].result == "已完成") {
            this.$notify({ type: "primary", message: "订单已经评价过了哦！" });
          }
        }
      }
    },
